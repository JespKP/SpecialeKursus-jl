...
# ── effective_stress ─────────────────────────────────────────────────────────
# Entry point: SpecialeKursus.effective_stress(σ)
@testset "effective_stress" begin
    # Uniaxial: σ_e = |σ_11|
    σ = SymmetricTensor{2,2}((100.0, 0.0, 0.0))
    @test SpecialeKursus.effective_stress(σ) ≈ 100.0

    # Pure shear: σ_e = √3/2 * τ  (von Mises)
    τ = 50.0
    σ = SymmetricTensor{2,2}((0.0, τ, 0.0))
    @test SpecialeKursus.effective_stress(σ) ≈ sqrt(3) * τ

    # Zero stress
    @test SpecialeKursus.effective_stress(zero(SymmetricTensor{2,2})) ≈ 0.0

    # Equibiaxial (σ_11 = σ_22, no shear): σ_e = σ_11
    σ = SymmetricTensor{2,2}((200.0, 0.0, 200.0))
    @test SpecialeKursus.effective_stress(σ) ≈ 200.0
end

# ── yield_function ───────────────────────────────────────────────────────────
# Entry point: SpecialeKursus.yield_function(σ, σ_y)
@testset "yield_function" begin
    ...
    @test SpecialeKursus.yield_function(σ_el, σ_y0) < 0
    @test SpecialeKursus.yield_function(σ_pl, σ_y0) > 0
    @test SpecialeKursus.yield_function(σ_on, σ_y0) ≈ 0.0
end

# ── flow_direction ───────────────────────────────────────────────────────────
# Entry point: SpecialeKursus.flow_direction(σ)
...
@testset "flow_direction" begin
    ...
    n = SpecialeKursus.flow_direction(σ)
    ...
    @test SpecialeKursus.flow_direction(zero(SymmetricTensor{2,2})) == zero(SymmetricTensor{2,2})
end

# ── compute_Δλ ───────────────────────────────────────────────────────────────
# Entry point: SpecialeKursus.compute_Δλ(σ_prev, Δε, mat, h)
...
@testset "stress_update!" begin
    @testset "elastic step stays inside yield surface" begin
        ...
        @test state.Δλ == 0.0
        @test SpecialeKursus.yield_function(state.σ, state.σ_y) < 0.0
    end
...
@testset "plastic step: explicit plastic increment update" begin
    ...
    n = SpecialeKursus.flow_direction(σ_prev)
    Δλ_expected = SpecialeKursus.compute_Δλ(σ_prev, Δε, mat, h)
